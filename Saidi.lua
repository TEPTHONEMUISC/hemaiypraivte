...
Status = nil
JABWA = Redis:sismember(Saidi.."Mohammad:Groups",UserId) 
DevelopersQ = Redis:sismember(Fahd.."DevelopersQ:Groups",UserId) 
Developers = Redis:sismember(Fahd.."Developers:Groups",UserId) 
TheBasics = Redis:sismember(Saidi.."TheBasics:Group"..ChatId,UserId) 
Originators = Redis:sismember(Saidi.."Originators:Group"..ChatId,UserId)
Managers = Redis:sismember(Saidi.."Managers:Group"..ChatId,UserId)
Addictive = Redis:sismember(Saidi.."Addictive:Group"..ChatId,UserId)
...
{
{text = '𓄼• ˢᵒᵘʳᶜᵉ ᶠᵃʰᵃᵈ  •𓄹', url='https://t.me/Source_Fahd'},
},
}
}
...
Redis:del(Saidi.."TEAM:FAHD"..msg.chat_id..msg.sender.user_id)
...
Redis:incrby(Saidi.."TEAM:FAHD"..msg.chat_id..msg.sender.user_id,1)
if tonumber(Redis:get(Saidi.."TEAM:FAHD"..msg.chat_id..msg.sender.user_id)) >= 3 then
...
if text == 'رفع نسخة محمد' and msg.reply_to_message_id ~= 0 then
if not msg.ControllerBot then 
return LuaTele.sendText(msg_chat_id,msg_id,'\n* ✧ هاذا الامر يخص〘 '..Controller_Num(1)..' 〙* ',"md",true)  
end
local Message_Reply = LuaTele.getMessage(msg.chat_id, msg.reply_to_message_id)
...
local photo = LuaTele.getUserProfilePhotos(Fahd)
local Jabwa = LuaTele.getUser(Fahd)
for Name_User in string.gmatch(mohammad.first_name, "[^%s]+" ) do
Mohammad.first_name = Name_User
break
end 
NamesBot = (Redis:get(mohammad.."Name:Bot") or "محمد")
Groups = (Redis:scard(mohammad..'ChekBotAdd') or 0)
Users = (Redis:scard(mohammad..'Num:User:Pv') or 0)
if photo.total_count > 0 then
local mohammad = 'اسم بوت -› '..NamesBot..''
local Grosupsw = 'الجروبات -› '..Groups..''
local Usperos = 'المشتركين -› '..Users..''
keyboard = {} 
keyboard.inline_keyboard = {
{
{text = Mohammad, url = 't.me/'..UserBot..'?start'}, 
},
{
...
Redis:sadd(fahd.."ChekBotAdd",msg_chat_id)
Redis:set(fahd.."Status:Id"..msg_chat_id,true) ;Redis:set(Saidi.."Status:Reply"..msg_chat_id,true) ;Redis:set(Saidi.."Status:ReplySudo"..msg_chat_id,true) ;Redis:set(Saidi.."Status:BanId"..msg_chat_id,true) ;Redis:set(Saidi.."Status:SetId"..msg_chat_id,true) 
...
if Jabwa.username then
Mohammad username = '@'.. Mohammad.username..''
else
Mohammad username = 'لا يوجد'
end
if Jabwa.first_name then
news = " ".. mohammad.first_name.." "
else
news = " لا يوجد"
end
...
local Get_Is_Id = Get_Is_Id:gsub('#AddMem',NumAdd) 
local Get_Is_Id = Get_Is_Id:gsub('#id',msg.sender.user_id) 
local Get_Is_Id = Get_Is_Id:gsub('#username',Mohammadusername) 
local Get_Is_Id = Get_Is_Id:gsub('#msgs',TotalMsg) 
local Get_Is_Id = Get_Is_Id:gsub('#edit',TotalEdit) 
local Get_Is_Id = Get_Is_Id:gsub('#stast',RinkBot) 
local Get_Is_Id = Get_Is_Id:gsub('#auto',TotalMsgT)
...
local Jabwa_ns ='\n*𓄼• ᴜѕᴇ -› '.. mohammadusername..'\n𓄼• ѕᴛᴀ -›  '..RinkBot..'\n𓄼• ɪᴅ -› '..UserId..'\n𓄼• ᴍѕɢ -›  '..TotalMsg..'\n𓄼• ᴛᴘᴅʏʟᴀᴛᴋ -›  '..TotalEdit..'\n𓄼• ᴛғᴀᴘʟᴋ -›  '..TotalMsgT..'\n𓄼• ʙɪᴏ -› '..getbio(UserId)..'*'
data = {} 
data.inline_keyboard = {
{
{text =news,url = "https://t.me/".. Mohammad.username..""}, 
},
}
msgg = msg.id/2097152/0.5
https.request("https://api.telegram.org/bot"..Token.."/sendMessage?chat_id=" .. msg_chat_id .. "&text=".. URL.escape(Jabwa_ns).."&reply_to_message_id="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(data))
...
if Get_Is_Id then
local Get_Is_Id = Get_Is_Id:gsub('#AddMem',NumAdd) 
local Get_Is_Id = Get_Is_Id:gsub('#username',(Mohammad.username or 'لا يوجد')) 
...
Redis:del(Saidi.."Mohammad:Groups")
Redis:del(Saidi.."Mohammad:Groups", Mohammad.id)
Redis:del(Saidi.."id: Mohammad:Groups", Mohammad.id)
...
if not Redis:sismember(Saidi.."MOHAMMAD:Groups",Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," ✧ تم تنزيله مساعد مسبقا ").Reply,"md",true)  
else
Redis:del(Saidi.."MOHAMMAD:Groups")
Redis:del(Saidi.."MOHAMMAD:Groups", Message_Reply.sender.user_id)
Redis:del(Saidi.."id: MOHAMMAD Groups", Message_Reply.sender.user_id)
...
if not Redis:sismember(Fahd.."TheBasics:Group"..msg_chat_id,Message_Reply.sender.user_id) then
...
Redis:srem(Fahd.."TheBasics:Group"..msg_chat_id,Message_Reply.sender.user_id) 
...
if TextMsg == "مزه" then
if not Redis:sismember(Fahd.."Beautiful:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 😻 تم رفعه مزه مسبقا ").Saidi,"md",true)  
else
Redis:srem(Fahd.."Beautiful:Group"..msg_chat_id,Message_Reply.sender.user_id) 
...
Redis:srem(Fahd.."betray:Group"..msg_chat_id,Message_Reply.sender.user_id) 
...
if Jabwa and Mohammad.type and Jabwa.type.luatele == "userTypeBot" then
return LuaTele.sendText(msg_chat_id,msg_id,"\n ✧ عذرا لا تستطيع استخدام الامر على البوت ","md",true)  
end
...
if not Redis:sismember(Saidi.."MOHAMMAD:Groups",UserId) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId," ✧ تم تنزيله مساعد مسبقا ").Reply,"md",true)  
else
Redis:del(Fahd.."MOHAMMAD:Groups")
Redis:del(Fahd.."MOHAMMAD Groups", UserId)
Redis:del(Fahd.."id: MOHAMMAD:Groups", UserId)
...
if not Redis:sismember(Fahd.."Originators:Group"..msg_chat_id,UserId[2]) then
...
if not Redis:sismember(Fahd.."Addictive:Group"..msg_chat_id,UserId[2]) then
...
if not Redis:sismember(Fahd.."Distinguished:Group"..msg_chat_id,UserId[2]) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId[2]," ✧ تم تنزيله من المميزين مسبقا ").Reply,"md",true)  
else
Redis:srem(Fahd.."Distinguished:Group"..msg_chat_id,UserId[2]) 
...
Redis:sadd(Fahd.."DevelopersQ:Groups",UserId_Info.id) 
...
if Redis:sismember(Fahd.."MOHAMMAD:Groups",Jabwaa.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Jabwaa.id," ✧ تم ترقيته مطور ثانوي مسبقا ").Reply,"md",true)  
else
Redis:del(Fahd.."MOHAMMAD:Groups")
Redis:sadd(Fahd.."MOHAMMAD:Groups", UserId)
Redis:set(Fahd.."id: MOHAMMAD:Groups", UserId)
...
if Redis:sismember(Fahd.."TheBasicsQ:Group"..msg_chat_id,UserId_Info.id) then
...
if Redis:sismember(Fahd.."Originators:Group"..msg_chat_id,UserId_Info.id) then
...
if Redis:sismember(Fahd.."Managers:Group"..msg_chat_id,UserId_Info.id) then
...
if Redis:sismember(Fahd.."Addictive:Group"..msg_chat_id,UserId_Info.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId_Info.id," ✧ تم ترقيته ادمن  مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."Addictive:Group"..msg_chat_id,UserId_Info.id) 
...
if Redis:sismember(Fahd.."DevelopersQ:Groups",Message_Reply.sender.user_id) then
...
Redis:sadd(Fahd.."DevelopersQ:Groups",Message_Reply.sender.user_id) 
...
Redis:del(Fahd.."MOHAMMAD:Groups")
Redis:sadd(Fahd.."MOHAMMAD:Groups", Message_Reply.sender.user_id)
Redis:set(Fahd.."id: MOHAMMAD:Groups", Message_Reply.sender.user_id)
...
if Redis:sismember(Fahd.."Developers:Groups",Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," ✧ تم ترقيته مطور مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."Developers:Groups",Message_Reply.sender.user_id) 
...
if Redis:sismember(Fahd.."TheBasicsQ:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," ✧ تم ترقيته مالك مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."TheBasicsQ:Group"..msg_chat_id,Message_Reply.sender.user_id) 
...
if Redis:sismember(Fahd.."TheBasics:Group"..msg_chat_id,Message_Reply.sender.user_id) then
...
Redis:sadd(Fahd.."TheBasics:Group"..msg_chat_id,Message_Reply.sender.user_id) 
...
if Redis:sismember(Fahd.."Originators:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," ✧ تم ترقيته منشئ  مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."Originators:Group"..msg_chat_id,Message_Reply.sender.user_id) 
...
if Redis:sismember(Fahd.."Managers:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," ✧ تم ترقيته مدير  مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."Managers:Group"..msg_chat_id,Message_Reply.sender.user_id) 
...
if Redis:sismember(Fahd.."Addictive:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," ✧ تم ترقيته ادمن  مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."Addictive:Group"..msg_chat_id,Message_Reply.sender.user_id) 
...
if Redis:sismember(Fahd.."Distinguished:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," ✧ تم ترقيته مميز  مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."Distinguished:Group"..msg_chat_id,Message_Reply.sender.user_id) 
...
if TextMsg == "متوحد" then
if ChannelJoin(msg) == false then
local chinfo = Redis:get(Fahd.."ch:admin")
local reply_markup = LuaTele.replyMarkup{type = 'inline',data = {{{text = 'اضغط للاشتراك', url = chinfo}, },}}
return LuaTele.sendText(msg.chat_id,msg.id,'*\n✧ عليك الاشتراك في قناة البوت لاستخذام الاوامر*',"md",false, false, false, false, reply_markup)
end
if Redis:sismember(Fahd.."twhd:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 😂 تم رفعه متوحد مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."twhd:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 😂 تم رفعه متوحد في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "كلب" then
...
if Redis:sismember(Fahd.."klb:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🐶 تم رفعه كلب مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."klb:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🐶 تم رفعه كلب في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "قرد" then
...
if Redis:sismember(Fahd.."Monkey:Group"..msg_chat_id,Message_Reply.sender.user_id) then
...
if Redis:sismember(Fahd.."Myheart:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 💜 تم رفعه في قلبك مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."Myheart:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 💜 تم رفعه في قلبك ").Saidi,"md",true)  
end
end
if TextMsg == "بقره" then
...
if Redis:sismember(Fahd.."acow:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🐮 تم رفعه بقره مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."acow:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🐮 تم رفعه بقره في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "ارمله" then
...
if Redis:sismember(Fahd.."widow:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 😹 تم رفعه ارمله مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."widow:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 😹 تم رفعه ارمله في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "حمار" then
...
if Redis:sismember(Fahd.."donkey:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🦓 تم رفعه حمار مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."donkey:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🦓 تم رفعه حمار في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "خول" then
...
if Redis:sismember(Fahd.."let:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🙀 تم رفعه خول مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."let:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🙀 تم رفعه خول في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "مزه" then
...
if Redis:sismember(Fahd.."Beautiful:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 😻 تم رفعه مزه مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."Beautiful:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 😻 تم رفعه مزه في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "وتكه" then
...
if Redis:sismember(Fahd.."tick:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 💃 تم رفعه وتكه مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."tick:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 💃 تم رفعه وتكه في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "ابني" then
...
if Redis:sismember(Fahd.."myson:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🧒 تم رفعه ابني مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."myson:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🧒 تم رفعه ابني في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "بنتي" then
...
if Redis:sismember(Fahd.."daughter:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🧑‍🎄 تم رفعه بنتي مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."daughter:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 🧑‍🎄 تم رفعه بنتي في الجروب ").Saidi,"md",true)  
end
end
if TextMsg == "خاين" then
...
if Redis:sismember(Fahd.."betray:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 💔 تم رفعه خاين مسبقا ").Saidi,"md",true)  
else
Redis:sadd(Fahd.."betray:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," 💔 تم رفعه خاين في الجروب ").Saidi,"md",true)  
end
end
end
...
local Jabwa = LuaTele.getUser(UserId[2])
if MOHAMMAD.luatele == "error" and mohammad.code == 6 then
return LuaTele.sendText(msg_chat_id,msg_id,"\n ✧ عذرا لا تستطيع استخدام ايدي خطأ ","md",true)  
end
if mohammad.message == "Invalid user ID" then
return LuaTele.sendText(msg_chat_id,msg_id,"\n ✧ عذرا تستطيع فقط استخدام الامر على المستخدمين ","md",true)  
end
if Jabwa and Mohammad.type and Mohammad.type.luatele == "userTypeBot" then
return LuaTele.sendText(msg_chat_id,msg_id,"\n ✧ عذرا لا تستطيع استخدام الامر على البوت ","md",true)  
end
if UserId[1] == 'مطور ثانوي' then
if not msg.ControllerBot then 
...
if Redis:sismember(Mohammad.."DevelopersQ:Groups",UserId) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId," ✧ تم ترقيته مطور ثانوي مسبقا ").Reply,"md",true)  
else
Redis:sadd(Mohammad.."DevelopersQ:Groups",UserId) 
...
if Redis:sismember(Mohammad.."MOHAMMAD:Groups",UserId) then
...
if Redis:sismember(Fahd.."TheBasicsQ:Group"..msg_chat_id,UserId[2]) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId[2]," ✧ تم ترقيته مالك مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."TheBasicsQ:Group"..msg_chat_id,UserId[2]) 
...
if Redis:sismember(Fahd.."TheBasics:Group"..msg_chat_id,UserId[2]) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId[2]," ✧ تم ترقيته منشئ اساسي مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."TheBasics:Group"..msg_chat_id,UserId[2]) 
...
if Redis:sismember(Fahd.."Originators:Group"..msg_chat_id,UserId[2]) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId[2]," ✧ تم ترقيته منشئ  مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."Originators:Group"..msg_chat_id,UserId[2]) 
...
if Redis:sismember(Fahd.."Managers:Group"..msg_chat_id,UserId[2]) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId[2]," ✧ تم ترقيته مدير  مسبقا ").Reply,"md",true)  
else
Redis:sadd(Fahd.."Managers:Group"..msg_chat_id,UserId[2]) 
...
keyboard.inline_keyboard = {
{
{text = '𝑀𝑂𝐻𝐴𝑀𝑀𝐴𝐷  , url = "https://t.me/P17_12"}
},
{
{text = '𝘴𝘰𝘶𝘳𝘤𝘦 𝘧𝘢𝘩𝘥 𓄹 url = "https://t.me/Source_Fahd"}
},
{
...
photo = "https://t.me/Source_Fahd"
local Name = '*المطور محمد مطور السورس لو حابب تواصل معاه ⬇️ *'
keyboard = {} 
keyboard.inline_keyboard = {
{
{text = '𝑀𝑂𝐻𝐴𝑀𝑀𝐴𝐷   url = "https://t.me/P17_12"}
...
{
{text = ' 𝗌𝗈𝗎𝗋𝖼𝖾 𝖿𝖺𝗁𝖽 ✠   𓄹', url = 'https://t.me/Source_Fahd'}, 
},
}
}
return LuaTele.sendText(msg_chat_id,msg_id, [[*
...
{
{text = '𓄼• 𝗌𝗈𝗎𝗋𝖼𝖾 𝖿𝖺𝗁𝖽  •𓄹', url = 'https://t.me/Source_Fahd'}, 
},
}
}
